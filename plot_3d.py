...
def plot_3d(model, preprocessor, X, y):
    ...
    X_sc_all    = preprocessor.scaler.transform(X.fillna(X.median()))
    ...
    y_vals = X["TotalSF"].values

    # Regression surface (2-feature for visualisation)
    ...
    fig.suptitle("3D House Price Visualisation — MLR (8 Features)",
                 fontsize=14, fontweight="bold", y=1.01)

    # Panel 1 — Actual prices + surface
    ax1 = fig.add_subplot(131, projection="3d")
    sc1 = ax1.scatter(x_vals, y_vals, actual_price/1000,
                      c=actual_price/1000, cmap="plasma", s=12, alpha=0.5)
    ax1.plot_surface(xx, yy, zz/1000, alpha=0.25, color="deepskyblue", edgecolor="none")
    ax1.set_xlabel("Overall Quality"); ax1.set_ylabel("Total SF")
    ax1.set_zlabel("Price ($K)"); ax1.set_title("Actual Prices\n+ Regression Surface")
    ...
    sc2 = ax2.scatter(x_vals, y_vals, predicted_price/1000,
                      c=predicted_price/1000, cmap="viridis", s=12, alpha=0.5)
    ax2.plot_surface(xx, yy, zz/1000, alpha=0.25, color="coral", edgecolor="none")
    ax2.set_xlabel("Overall Quality"); ax2.set_ylabel("Total SF")
    ax2.set_zlabel("Predicted Price ($K)"); ax2.set_title("Predicted Prices\n+ Regression Surface")
    ...
    sc3 = ax3.scatter(actual_price/1000, predicted_price/1000, error_pct,
                      c=error_pct, cmap="RdYlGn_r", vmin=-40, vmax=40, s=12, alpha=0.6)
    lim = [min(actual_price.min(), predicted_price.min())/1000*0.95,
           max(actual_price.max(), predicted_price.max())/1000*1.05]
    ax3.plot(lim, lim, [0, 0], color="black", lw=1.5, linestyle="--", label="Perfect fit")
    ax3.set_xlabel("Actual ($K)"); ax3.set_ylabel("Predicted ($K)")
    ax3.set_zlabel("Error (%)"); ax3.set_title("Actual vs Predicted\nColoured by % Error")
    fig.colorbar(sc3, ax=ax3, shrink=0.45, pad=0.1, label="Error (%)")
    ax3.legend(fontsize=7); ax3.view_init(elev=22, azim=45)
